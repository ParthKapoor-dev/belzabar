...
const releaseCommandsDir = join(process.cwd(), "../integrations/release/commands");
...
const migrateCommands = discoverCommands(migrationsCommandsDir);
const releaseCommands = discoverCommands(releaseCommandsDir);
...
const migrateRegistry =
  ...
  migrateCommands.map(cmd => `  "${cmd}": ${toIdentifier(cmd)},`).join("\n") +
  `\n};\n`;

const migrateContent = `// Auto-generated by utils/generate-registry.ts
// Do not edit manually.
${migrateImports}
...
// ── registry-release.ts ──────────────────────────────────────────────────────
const releaseImports = releaseCommands
  .map(cmd => `import * as ${toIdentifier(cmd)} from "../../integrations/release/commands/${cmd}/index";`)
  .join("\n");
const releaseRegistry =
  `export const ReleaseCommandRegistry: Record<string, any> = {\n` +
  releaseCommands.map(cmd => `  "${cmd}": ${toIdentifier(cmd)},`).join("\n") +
  `\n};\n`;

const releaseContent = `// Auto-generated by utils/generate-registry.ts
// Do not edit manually.
${releaseImports}
...
const migrateHelpMap = buildHelpMap(migrateCommands, migrationsCommandsDir);
const releaseHelpMap = buildHelpMap(releaseCommands, releaseCommandsDir);
...
const helpFullText = buildHelpFull([
  ...
  { header: "belz migrate <cmd>  —  Migrations (Jenkins)", commands: migrateCommands, dir: migrationsCommandsDir },
  { header: "belz release <cmd>  —  Release promotion tracking", commands: releaseCommands, dir: releaseCommandsDir },
...
export const MigrateHelpMap: Record<string, string> = ${migrateHelpMap};

export const ReleaseHelpMap: Record<string, string> = ${releaseHelpMap};
...
await Bun.write(join(topCommandsDir, "registry-migrate.ts"), migrateContent);
await Bun.write(join(topCommandsDir, "registry-release.ts"), releaseContent);
...
console.log(`✅ registry-migrate.ts (${migrateCommands.length} migrate commands: ${migrateCommands.join(", ")})`);
console.log(`✅ registry-release.ts (${releaseCommands.length} release commands: ${releaseCommands.join(", ")})`);
console.log(`✅ registry-help.ts    (embedded help text for ${adCommands.length + pdCommands.length + twCommands.length + topLevelCommands.length + migrateCommands.length + releaseCommands.length} commands)`);
