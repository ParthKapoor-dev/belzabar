...
export class DisplayManager {
  ...
  static configure(options: { llm: boolean }) {
    if (options.llm) {
      this.mode = 'LLM';
      // Optional: Silence standard console.log to keep output pure
      // console.log = () => {}; 
    }
  }
  ...
  static table(headers: string[], rows: any[][]) {
    if (this.mode === 'HUMAN') {
      ...
      // LLM Mode: Convert to array of objects
  ...
  // Helper for Key-Value pairs (Human: Text, LLM: Object)
  static kv(key: string, value: any) {
     if (this.mode === 'HUMAN') {
         console.log(`${chalk.bold(key)}: ${value}`);
     } else {
         // This is tricky for single lines, maybe accumulate?
         // For now, simple print object
